########################################################################################################################################
## Detect lineages for each country, pertussis
########################################################################################################################################
## Packages used
library(stringr)
library(ape)
...
potential_splits_France = readRDS(paste0('3_BordetellaPertussis/2_analysis_index/2_find_index_groups/potential_splits_weighting_France_tracked_ksmooth', k_smooth, '_min_group_size_', min_group_size, '.rds'))$best_nodes_names[[8]]
split_France = merge.groups(timed_tree = tree_France, metadata = dataset_with_nodes_France, 
                            initial_splits = potential_splits_France, 
                            group_count_threshold = 30, group_freq_threshold = 0)
######################################################################################################################################
...
dataset_with_nodes_France$groups = as.factor(split_France$groups)
name_groups_France = levels(dataset_with_nodes_France$groups)
## Reorder labels by time of emergence
name_groups_France = levels(dataset_with_nodes_France$groups)
time_groups_France = NULL
for(i in 1:length(name_groups_France)){
  time_groups_France = c(time_groups_France, min(dataset_with_nodes_France$time[which(dataset_with_nodes_France$groups == name_groups_France[i] &
                                                                                     dataset_with_nodes_France$is.node == 'yes')]))
}
levels(dataset_with_nodes_France$groups) = match(name_groups_France, order(time_groups_France, decreasing = T))
dataset_with_nodes_France$groups = as.numeric(as.character(dataset_with_nodes_France$groups))
dataset_with_nodes_France$groups = as.factor(dataset_with_nodes_France$groups)
## Update split
split_France$tip_and_nodes_groups = match(split_France$tip_and_nodes_groups, order(time_groups_France, decreasing = T))
names(split_France$tip_and_nodes_groups) = 1:length(split_France$tip_and_nodes_groups)
split_France$groups = as.factor(split_France$groups)
levels(split_France$groups) = match(name_groups_France, order(time_groups_France, decreasing = T))
split_France$groups = as.numeric(as.character(split_France$groups))
## Choose color palette
n_groups_France <- length(name_groups_France)
colors_groups_France = (met.brewer(name="Cross", n=n_groups_France, type="continuous"))
## Color each group
dataset_with_nodes_France$group_color = dataset_with_nodes_France$groups
levels(dataset_with_nodes_France$group_color) = colors_groups_France
dataset_with_nodes_France$group_color = as.character(dataset_with_nodes_France$group_color)
#######################################################################################################################################

#######################################################################################################################################
## Save the detection
#######################################################################################################################################
save.image('3_BordetellaPertussis/2_analysis_index/2_find_index_groups/Lineages_detected.Rdata')
#######################################################################################################################################
