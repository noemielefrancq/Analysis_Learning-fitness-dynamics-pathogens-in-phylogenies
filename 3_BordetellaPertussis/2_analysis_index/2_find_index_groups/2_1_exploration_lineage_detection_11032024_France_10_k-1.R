########################################################################################################################################
## Detect lineages for pertussis in France
########################################################################################################################################
## Packages used
library(ape)
library(phangorn)
...
max_stepwise_deviance_explained_threshold = -1
max_groups_found = 20
stepwise_AIC_threshold = -1000

keep_track = T
######################################################################################################################################

######################################################################################################################################
## Test detection lineages - France
######################################################################################################################################
start_time = Sys.time()
potential_splits_weighting_France = find.groups.by.index.dynamics(timed_tree = tree_France,
                                                                  metadata = dataset_with_nodes_France,
                                                                  node_support = tree_France$node.label,
...
saveRDS(potential_splits_weighting_France, paste0('3_BordetellaPertussis/2_analysis_index/2_find_index_groups/potential_splits_weighting_France_tracked_ksmooth', k_smooth, '_min_group_size_', min_group_size, '.rds'))
######################################################################################################################################
