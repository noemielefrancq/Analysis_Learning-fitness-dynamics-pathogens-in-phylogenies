...
colors_genotypes[which(colors_genotypes == 'grey60')] = 'white'

########################################################################################################################################
## Plot France comparison
########################################################################################################################################
genotypes_France = matrix(as.character(dataset_with_nodes_France$genotype), ncol = 1)
colnames(genotypes_France) = 'groups'
rownames(genotypes_France) = dataset_with_nodes_France$name_seq
plot_tree_France_genotypes <- ggtree(tree_France, mrsd=lubridate::date_decimal(max(dataset_tips_France$time)), size = 0.15,
                                    aes(color = as.character(dataset_with_nodes_France$genotype))) + 
  scale_color_manual(values = colors_genotypes)+
  theme_tree2()
plot_tree_France_genotypes = gheatmap(plot_tree_France_genotypes, genotypes_France, offset=0.1, width=0.10, 
...
groups_France = matrix(dataset_with_nodes_France$groups, ncol = 1)
colnames(groups_France) = 'groups'
rownames(groups_France) = dataset_with_nodes_France$name_seq
plot_tree_France_groups <- ggtree(tree_France, mrsd=lubridate::date_decimal(max(dataset_tips_France$time)), size = 0.15,
                                 aes(color = as.character(dataset_with_nodes_France$groups))) + 
  scale_color_manual(values = colors_groups_France)+
  theme_tree2()
plot_tree_France_groups = gheatmap(plot_tree_France_groups, groups_France, offset=0.1, width=0.10, 
                                  colnames=FALSE, legend_title="Group", color=NA) +
  scale_fill_manual(values = colors_groups_France)+
  scale_x_ggtree() + 
  scale_y_continuous(expand=c(0, 0.3))+
  theme(legend.position = 'none')

comparison_France = plot_grid(plot_tree_France_groups, plot_tree_France_genotypes, 
          rel_widths = c(1, 1), labels = '', ncol = 2)

ggsave(filename = '3_BordetellaPertussis/2_analysis_index/2_find_index_groups/Comparison_France_tree_groups_genotype.pdf', 
       plot = comparison_France, device = 'pdf', scale = 1,
       width = 7, height = 7, units = 'cm')
########################################################################################################################################


#######################################################################################################################################
## Plot heatmap groups vs known lineages
#######################################################################################################################################

#####################################################################
## France
#####################################################################
grouping1 = levels(as.factor(dataset_with_nodes_France$genotype))
grouping1 = grouping1[which(grouping1 != '4')] ## 4 is the NA
grouping2 = names(table(dataset_with_nodes_France$groups)[which(table(dataset_with_nodes_France$groups) > 0)])
...
for(i in 1:length(grouping1)){
  idx = which(dataset_with_nodes_France$genotype == grouping1[i])
  t = table(factor(as.character(dataset_with_nodes_France$groups[idx]), levels = grouping2))
  t = t/sum(t)
  correspondance_groups[match(names(t), grouping2), i] = t
}
correspondance_groups = correspondance_groups
...
correspondance_groups_auto = correspondance_groups[orders$rows,(orders$cols)]
...
idx = which(!is.na(dataset_with_nodes_France$genotype) & dataset_with_nodes_France$is.node == 'no')
group1 = as.numeric(dataset_with_nodes_France$genotype[idx])
group2 = dataset_with_nodes_France$groups[idx]
rand.treeannotator_partitions = fossil::adj.rand.index(group1, group2)
# rand.treeannotator_partitions 0.6278308
#######################################################################################################################################
